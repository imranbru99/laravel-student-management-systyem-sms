...
<div class="max-w-2xl mx-auto bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800 p-8">
    ...
    <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full px-4 py-3 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none transition" required>
            @error('name') <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span> @enderror
        </div>

        <div>
            <label class="block text-sm font-medium mb-2">Email Address</label>
            <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full px-4 py-3 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none transition" required>
        ...
        <div>
            ...
            <select name="role" class="w-full px-4 py-3 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none transition dark:text-zinc-100" required>
                <option value="" disabled class="dark:bg-zinc-900">Select a role...</option>
                @foreach($roles as $role)
                    <option value="{{ $role->name }}" class="dark:bg-zinc-900"
        ...
        <div class="flex justify-end gap-3 pt-6 border-t border-zinc-200 dark:border-zinc-800">
            ...
            <button type="submit" class="bg-violet-600 hover:bg-violet-700 text-white px-6 py-3 rounded-xl font-medium transition shadow-sm">Update User</button>
        </div>
    </form>
</div>
@endsection
