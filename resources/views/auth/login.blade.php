...
<head>
    ...
    <title>Log in - {{ config('app.name', 'EduManage') }}</title>
...
<body class="min-h-screen flex items-center justify-center relative overflow-hidden antialiased selection:bg-violet-500/30 p-6">
    ...
    <div class="w-full max-w-md">
        <div class="flex flex-col items-center mb-8">
            ...
            <p class="text-zinc-400 mt-2 text-sm">Enter your credentials to access your account</p>
        </div>
        ...
        <div class="bg-zinc-900/60 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-2xl shadow-black/50 relative overflow-hidden">
            ...
            @if (session('status'))
                <div class="mb-6 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-emerald-400 text-sm font-medium text-center">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-xs font-medium text-violet-400 hover:text-violet-300 transition-colors">
                                Forgot password?
                            </a>
                        @endif
                    </div>
                ...
                <div class="flex items-center pt-2">
                    <label for="remember_me" class="flex items-center cursor-pointer group">
                        <div class="relative flex items-center justify-center w-5 h-5 mr-3 border border-white/20 rounded bg-black/50 group-hover:border-violet-500 transition-colors">
                            <input id="remember_me" type="checkbox" name="remember" class="peer absolute opacity-0 w-full h-full cursor-pointer">
                            <i class="fas fa-check text-xs text-white opacity-0 peer-checked:opacity-100 transition-opacity"></i>
                            <div class="absolute inset-0 bg-violet-600 rounded opacity-0 peer-checked:opacity-100 -z-10 transition-opacity"></div>
                        </div>
                        <span class="text-sm text-zinc-400 group-hover:text-zinc-300 transition-colors">Remember me for 30 days</span>
                    </label>
                </div>
        ...
        <p class="text-center mt-8 text-sm text-zinc-500">
            ...
            <a href="{{ route('register') }}" class="text-white font-medium hover:text-violet-400 transition-colors underline underline-offset-4 decoration-white/20 hover:decoration-violet-400/50">
                Create one now
            </a>
        </p>
    </div>
